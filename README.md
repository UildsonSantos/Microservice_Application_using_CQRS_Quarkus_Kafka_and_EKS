...
Live coding by deploying an application written in Java / Kotlin on Amazon's Elastic Kubernetes Service with specialist Wesley Fuchter by DIO (digital innovation one).

The application is an example of the CQRS standard that includes two Quarkus services that communicate via an asynchronous bus using Kafka, allowing low cost for Java applications in the cloud.
